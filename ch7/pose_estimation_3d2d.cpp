...
int main(int argc, char **argv) {
  // Load image
  if (argc != 4) {
    cout << " wrong number of input, correct usage: pose_est_3d2d img1 img2 "
            "depth1"
         << endl;
    return 1;
  }
  ...
  assert(img_1.data != nullptr && img_2.data != nullptr &&
         depth_1.data != nullptr);  // check if images loaded successfully
  ...
  for (DMatch m : matches) {
    ushort d = depth_1.ptr<unsigned short>(
...
void bundleAdjustmentGaussNewton(const VecVector3d &points_3d,
  ...
  for (int iter = 0; iter < iterations; iter++) {
    ...
    cout << "iteration " << iter << " cost=" << std::setprecision(12) << cost
         << endl;
    if (dx.norm() < 1e-6) {
      // converge
      break;
...
// Here just copied from the original code, try to go through this next time
void bundleAdjustmentG2O(const VecVector3d &points_3d,
                         const VecVector2d &points_2d, const Mat &K,
                         Sophus::SE3d &pose) {
  // 构建图优化，先设定g2o
