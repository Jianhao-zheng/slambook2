 ...
 * @param T_C_R         transformation from reference to current
 * @param depth
 * @param depth_cov
 * @return
 */
bool update(const Mat &ref, const Mat &curr, const SE3d &T_C_R, Mat &depth,
            Mat &depth_cov2);
 ...
 * @param curr
 * @param T_C_R
 * @param pt_ref        pixel position in reference image
 * @param depth_mu
 * @param depth_cov
 * @param pt_curr
...
bool readDatasetFiles(const string &path, vector<string> &color_image_files,
  ...
  while (!fin.eof()) {
    // data format: image_name tx, ty, tz, qx, qy, qz, qw ，(note: here it's
    // T_WC)
    string image;
    fin >> image;
    double data[7];
  ...
  for (int x = boarder; x < width - boarder; x++)
    for (int y = boarder; y < height - boarder; y++) {
      if (depth_cov2.ptr<double>(y)[x] < min_cov ||
          depth_cov2.ptr<double>(y)[x] > max_cov)  // converge or diverge
        continue;
      ...
      updateDepthFilter(Vector2d(x, y), pt_curr, T_C_R, epipolar_direction,
                        depth, depth_cov2);
    }
}
...
bool epipolarSearch(const Mat &ref, const Mat &curr, const SE3d &T_C_R,
  ...
  if (half_length > 100) half_length = 100;  // limit the number of searches
  ...
  for (double l = -half_length; l <= half_length; l += 0.7) {  // l+=sqrt(2)
    Vector2d px_curr =
        px_mean_curr + l * epipolar_direction;  // to be evaluated
  ...
  if (best_ncc < 0.85f)  // hard threshold
    return false;
...
bool updateDepthFilter(const Vector2d &pt_ref, const Vector2d &pt_curr,
  ...
  // f2 = R_RC * f_cur
  // convert to the following equation
  ...
  Vector3d xm = ans[0] * f_ref;   // estimated by reference
  Vector3d xn = t + ans[1] * f2;  // estimated by current
...
void evaludateDepth(const Mat &depth_truth, const Mat &depth_estimate) {
  ...
  cout << "Average squared error = " << ave_depth_error_sq
       << ", average error: " << ave_depth_error << endl;
}
