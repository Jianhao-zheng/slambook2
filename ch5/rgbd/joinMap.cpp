...
int main(int argc, char **argv) {
  ...
  pose_file = folder + pose_file;
  ifstream fin(pose_file);
  if (!fin) {
    cerr << "file \"" << pose_file
         << "\" does not exist. Please give a correct folder path." << endl;
    return 1;
  }

  for (int i = 0; i < 5; i++) {
    boost::format fmt(folder + "/%s/%d.%s");
  ...
  for (int i = 0; i < 5; i++) {
    ...
    for (int v = 0; v < color.rows; v++)
      for (int u = 0; u < color.cols; u++) {
        ...
        if (d == 0) continue;  // no measurement if 0
  ...
  cout << "num of points is: " << pointcloud.size() << endl;
