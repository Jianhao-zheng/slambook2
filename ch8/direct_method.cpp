...
int main(int argc, char **argv) {
  ...
  if (argc != 3) {
    cout
        << " wrong number of input, correct usage: ./direct_method ../left.png "
           "../disparity_img.png"
        << endl;
    return 1;
  }
  Mat left_img = imread(argv[1], 0);
  Mat disparity_img = imread(argv[2], 0);
  assert(left_img.data != nullptr &&
         disparity_img.data != nullptr);  // check if images loaded successfully
  ...
  for (int i = 0; i < nPoints; i++) {
    int x = rng.uniform(
        boarder,
        left_img.cols - boarder);  // don't pick pixels close to boarder
    int y = rng.uniform(
        boarder,
        left_img.rows - boarder);  // don't pick pixels close to boarder
