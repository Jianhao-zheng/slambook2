...
int main(int argc, char **argv) {
  // Load image
  if (argc != 3) {
    cout << " wrong number of input, correct usage: ./optical_flow ../LK1.png "
            "../LK2.png"
         << endl;
    return 1;
  }
  ...
  assert(img1.data != nullptr &&
         img2.data != nullptr);  // check if images loaded successfully

  // follow the author, using GFTT key points here.
  vector<KeyPoint> kp1;
  Ptr<GFTTDetector> detector =
      GFTTDetector::create(500, 0.01, 20);  // maximum 500 keypoints
  ...
  cout << "optical flow by gauss-newton (single-level): " << time_used.count()
       << endl;
  ...
  t2 = chrono::steady_clock::now();
  time_used = chrono::duration_cast<chrono::duration<double>>(t2 - t1);
  cout << "optical flow by gauss-newton (multi-level): " << time_used.count()
       << endl;
